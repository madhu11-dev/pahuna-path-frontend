...
const AdminPlaces = () => {
  ...
  const handleVerifyPlace = async (placeId, placeName, currentStatus) => {
    try {
      const response = await verifyPlaceApi(placeId);
      if (response.status) {
  ...
  const executeMerge = async () => {
    ...
    try {
      ...
      if (response.status) {
        toast.success(
          `Places merged successfully into "${response.data.place_name}"`
        );
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {places.map((place) => (
              <div
                ...
                onClick={
                  mergeMode ? () => handlePlaceSelection(place.id) : undefined
                }
              >
                ...
                <div className="p-5">
                  <div className="flex justify-between items-start mb-3">
                    ...
                    {!mergeMode && (
                      ...
                      >
                        ...
                        {place.is_verified ? "Verified" : "Verify"}
                      </button>
                    )}
                  </div>
                  ...
                  {!mergeMode && (
                    <div className="flex space-x-2">
                      ...
                      <button
                        onClick={() =>
                          handleDeletePlace(place.id, place.place_name)
                        }
                          ...
                          return place ? (
                            ...
                            >
                              <input
                                ...
                                checked={
                                  mergeOptions.selectedPlaceName ===
                                  place.place_name
                                }
                                onChange={(e) =>
                                  setMergeOptions({
                          ...
                          return place ? (
                            ...
                            >
                              <input
                                ...
                                checked={
                                  mergeOptions.selectedDescription ===
                                  place.description
                                }
                                onChange={(e) =>
                                  setMergeOptions({
                            ...
                            place?.images?.map((image, idx) => (
                              <div
                                ...
                                onClick={() => {
                                  const newImages =
                                    mergeOptions.selectedImages.includes(image)
                                      ? mergeOptions.selectedImages.filter(
                                          (img) => img !== image
                                        )
                              ...
                              >
                                ...
                                />
                                {mergeOptions.selectedImages.includes(
                                  image
                                ) && (
